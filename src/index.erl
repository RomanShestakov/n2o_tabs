...
templates() -> code:priv_dir(n2o_tabs) ++ "/templates".

title() -> "Nitrogen Elements Examples".
headline() -> "Nitrogen Elements Examples".

-define(TABS_ELEMENT, #tabs{}).
-define(TABS_ID, tabs_id).

body() ->

    #panel{id=n2ostatus},

    wf:wire(#event{trigger = ?TABS_ID, type = ?EVENT_TABS_ACTIVATE, postback = {?TABS_ID, ?EVENT_TABS_ACTIVATE}, delegate = ?TABS_ELEMENT#tabs.module}),
    %% output html markup
    [
    	#tabs{
    	    id = ?TABS_ID,
